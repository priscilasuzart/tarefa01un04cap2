...
{
    stdio_init_all(); //Inicializa o sistema de entrada/saída padrão no Raspberry Pi Pico
    inicia_teclado(); //Função definida anteriormente para inicializar os GPIOS do teclado matricial

        while (1) { // Criação de loop infinito
        char tecla = ler_teclado(); //Retorna um valor do tipo char, que será armazenado na variável tecla
        

        switch(tecla){ //Verifica qual tecla foi pressionada e executa uma ação específica dependendo do valor armazenado em tecla
                


            case 'A': //Se tecla pressionada for A
            liga_BOTAO_A ();
            printf("Botão A pressionado\n");
            break;

            case 'B': //Se tecla pressionada for B
            liga_BOTAO_B();
            printf("Botão B pressionado\n");
            break;

            case 'C': //Se tecla pressionada for C
            ligarledverde();
            printf("Botão C pressionado\n");
            break;
                       
            case 'D': //Se tecla pressionada for D
            ligarLeds();
            printf("Botão D pressionado\n");
            break;

            case '#': //Se tecla pressionada for #
            liga_BUZZER();
            printf("Botão # pressionado\n");
            break;

            case '1': //Se tecla pressionada for 1
            liga_azulVerde();
            printf("Botão 1 pressionado\n");
            break;

            case '2': //Se tecla pressionada for 2
            liga_VermelhoVerde();
            printf("Botão 2 pressionado\n");
            break;

            case '3': //Se tecla pressionada for 3
            tecla3_liga();
            printf("Botão 3 pressionado\n");
            break;           

            case '\0': //Se não for pressionada nenhuma tecla
            desligarLeds();
            printf("Nenhuma tecla pressionada\n");
            break;

            default: //Se não corresponder a nenhum dos casos acima
            break;
       
        }
        sleep_ms(1000); //Introduz um atraso de 1 segundo antes de recomeçar o loop e verificar novamente o teclado
    }
}
